...
@section('sidebar')
	<ul>
		<li><a href="" class="heading">USERS LIST</a></li>
	</ul>
	<div class="pull-right" style="margin: 0 20px 0 0;">
		<a href="{{route('bank.user.add')}}"><button type="submit" class="btn btn-primary btn-sm">Create New User</button></a>
	</div>
@stop
@section('content')
			...
			@if(isset($users))
			   @foreach($users as $user)
				<tr>
				  ...
				  <td>
					<a href="{{route('bank.user.edit',[$user->uuid])}}" class="btn btn-warning btn-xs"><i class="fa fa-edit" title="Edit"></i></a>
					@if($user->status === "1")
