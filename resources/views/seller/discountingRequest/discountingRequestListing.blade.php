...
@section('sidebar')
	<ul>
		<li><a href="" class="heading">DISCOUNTING REQUEST </a></li>
  </ul>
@stop
@section('content')
 <!-- Row Start -->
                   ...
                   @endif
                    <form class="form-horizontal ng-pristine ng-valid" role="form" name="search_poi">
                      <div class="form-group">
                        ...
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-2 demo"> <input type="text" name="invoiceDate" id="config-demo" class="form-control date_filter" value="{{ Input::get('invoiceDate')}}">
                          <i class="glyphicon glyphicon-calendar fa fa-calendar"></i>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-2">  
                         <select name="sorting" class="form-control ng-pristine ng-untouched ng-invalid ng-invalid-required lstFruits">
                           <option value="">Please Select</option>
                           <option value="invoiceNumber-ASC" @if(Input::has('sorting') && (Input::get('sorting') === 'invoiceNumber-ASC')) selected @endif> Ascending Invoice Number</option>
                           <option value="invoiceNumber-DESC" @if(Input::has('sorting') && (Input::get('sorting') === 'invoiceNumber-DESC')) selected @endif> Descending Invoice Number</option>
                           <option value="loanAmount-ASC" @if(Input::has('sorting') && (Input::get('sorting') === 'loanAmount-ASC')) selected @endif> Ascending Eligible Amount</option>
                           <option value="loanAmount-DESC" @if(Input::has('sorting') && (Input::get('sorting') === 'loanAmount-DESC')) selected @endif> Descending Eligible Amount</option>
                           <option value="buyerName-ASC" @if(Input::has('sorting') && (Input::get('sorting') === 'buyerName-ASC')) selected @endif> Ascending Buyer Name</option>
                           <option value="buyerName-DESC" @if(Input::has('sorting') && (Input::get('sorting') === 'buyerName-DESC')) selected @endif> Descending Buyer Name</option>
                           <option value="dueDate-ASC" @if(Input::has('sorting') && (Input::get('sorting') === 'dueDate-ASC')) selected @endif> Ascending Due Date</option>
                           <option value="dueDate-DESC" @if(Input::has('sorting') && (Input::get('sorting') === 'dueDate-DESC')) selected @endif> Descending Due Date</option>
                           <option value="piAmount-ASC" @if(Input::has('sorting') && (Input::get('sorting') === 'piAmount-ASC')) selected @endif> Ascending PI Amount</option>
                           <option value="piAmount-DESC" @if(Input::has('sorting') && (Input::get('sorting') === 'piAmount-DESC')) selected @endif> Descending PI Amount</option>
                         </select>          
                        </div>
                        ...
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3 pull-right" style="border-left:1px solid #ccc;"> 
                          ...
                          <span class="col-sm-11 pull-right">
                            ...
                            <select name="buyer" class="form-control changeDiscountingStatus">
                              ...
                              <option value="1">Submit Request</option>
                      ...
                      <table class="table table-condensed table-striped table-bordered table-hover no-margin">
                        ...
                        @if(isset($iReqData))
                          ...
                          <tr>
                            ...
                            <td>
                            @if($approveRejectButton === "Y")
                            <a @if(in_array($valiReqData->status, array(1,3,2,4,6))) style="display:none" @endif href="javascript:void(0);" class="btn btn-success btn-xs discounting" data-original-title="Submit Request" data-disId="{{$valiReqData->uuid}}" data-statusId="1">
                              <i class="fa fa-check"></i>
                            </a>
                            <a @if(in_array($valiReqData->status, array(1,2,4,6))) style="display:none" @endif href="javascript:void(0);" class="btn btn-danger btn-xs discounting" data-original-title="Reject Request" data-disId="{{$valiReqData->uuid}}" data-statusId="2">
     ...
     <div class="modal fade" id="confirmRejectMoadal" role="dialog" aria-labelledby="confirmDeleteLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <form class="form-horizontal no-margin ng-valid-parse ng-invalid ng-invalid-required ng-valid-min ng-valid-pattern ng-pristine" role="form" method="post" action="{{route('seller.iDiscounting.postApprove')}}">
          {{ csrf_field() }}
